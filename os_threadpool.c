...
static void *thread_loop_function(void *arg)
{
	...
	while (1) {
		...
		if (t == NULL) {
			pthread_mutex_lock(&tp->lock);
			// in cazul in care s-ar face dequeue fara sa se fi adaugat ceva
			if (tp->start_work == 1)
				tp->wait_threads++;

			// daca este al n-lea thread care asteapta se opreste
			if (tp->wait_threads == (int)tp->num_threads) {
				tp->work_done = 1;
				pthread_cond_signal(&tp->cond);
			}
